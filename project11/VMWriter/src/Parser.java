...
public class Parser {
	...
	void openFilesAndExecute ()
	{
		...
		{				
			outputFileName = inputFileNames.get(i).replace(".jack", ".ours.vm");
	...
	private String insertWhiteSpaceIntoFullFileAsString(String trimmedLine) {
		JackTokenizer temp = new JackTokenizer();
		String i = temp.getASymbol();

		while(!(i.equals("done")))
		{
			if(trimmedLine.contains(i))
			{
				trimmedLine = trimmedLine.replace(i, " " +i+ " " );
			}
			i = temp.getASymbol();
		}
	...
	private void prepareInputStringAndStoreInJackLineOfCode(String readLine) {
		...
		trimmedLine = insertWhiteSpaceIntoFullFileAsString(trimmedLine);
		if(trimmedLine.contains("\""))
		{
			...
			while(!(i.equals("done")))
			{
				if(temp2.contains(i))
				{
					temp2 = temp2.replace("  " + i + "  ",i);
				}
				i = temp.getASymbol();
			}
			String tempSub = temp2.replace("  ", " ");
